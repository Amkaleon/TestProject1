...
def parse_header_banner(browser) -> List[Dict[str, Any]]:
    ...
    header_container = browser.find_element(By.XPATH, '//header[@role="banner"]/div[@data-testid="advContainer"]')

    try:
        # Получаем изображение с background-image в CSS-селекторе вложенного div
        image_url = header_container.find_element(By.XPATH, './/div/div/div').get_attribute('style')
        # Чтобы сильно не усложнять - разбил background-image по кавычкам. Можно еще через регулярные выражения.
    ...
    try:
        ...
        content_url = header_container.find_element(By.TAG_NAME, 'a').get_attribute('href')
    except NoSuchElementException:
